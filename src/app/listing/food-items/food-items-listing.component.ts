...
export class FoodItemsListingComponent implements OnInit {
  ...
  private fetchItemListingData(): void {
    this.preloaderService.show();
    this.dataService.get(API_FOOD_ITEM_LISTING)
  ...
  public onCategoryMenuEditClick(guid: string) {
  ...
  public onItemMenuEditBtnClick(guid: string) {
    const queryParameter = {
      [QUERY_PARAM_KEY_GUID]: guid
    }
    this.router.navigate([CREATE_FOOD_ITEM_PAGE], { queryParams: queryParameter })
  }
  ...
  public getItemQuantity(quantity: number, quantityUnit: FOOD_ITEM_QUANTITY_UNIT): string {
    let suffix: string = "";
    switch(quantityUnit) {
  ...
  public getItemType(itemType: FOOD_ITEM_TYPE): string {
    let typeString = "";
    switch(itemType) {
      case FOOD_ITEM_TYPE.VEG:
        typeString = "Veg";
        break;
        case FOOD_ITEM_TYPE.NON_VEG:
        typeString = "Non veg";
        break;
    }
